...
/**
 * ============================================
 * PAGE PORTFOLIO - AFFICHES
 * ============================================
 * 
 * Cette page affiche uniquement les affiches du portfolio
    ...
    <div class="portfolio-conteneur">
        <div class="filter-row">
            <div class="filter-buttons">
                ...
                <button onclick="window.location.href='portfolio-affiches.php'" class="filter-btn active">Affiches</button>
            </div>
        </div>
        ...
        <div class="portfolio-grid">
            <?php if (!empty($affiches)): ?>
                <?php foreach ($affiches as $affiche): ?>
                    <?php 
                    $imagePath = $affiche['image_path'] ?? '';
                    if (strpos($imagePath, 'admin/') !== 0 && strpos($imagePath, '/') !== 0) {
                        if (strpos($imagePath, 'images/') === 0) {
                            $imagePath = 'admin/' . $imagePath;
                        } else {
                            $imagePath = 'admin/images/Admin/affiches/' . basename($imagePath);
                        }
                    }
                    $altText = htmlspecialchars($affiche['client_name'] ?? 'Affiche', ENT_QUOTES, 'UTF-8');
                    ?>
                    <div class="item affiches">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p class="no-items">Aucune affiche disponible pour le moment.</p>
            <?php endif; ?>
        </div>
    </div>
</section>
