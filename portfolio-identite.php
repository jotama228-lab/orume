...
/**
 * ============================================
 * PAGE PORTFOLIO - IDENTITÉS VISUELLES
 * ============================================
 * 
 * Cette page affiche uniquement les identités visuelles du portfolio
    ...
    <div class="portfolio-conteneur">
        <div class="filter-row">
            <div class="filter-buttons">
                ...
                <button onclick="window.location.href='portfolio-identite.php'" class="filter-btn active">Identité visuelle</button>
                <button onclick="window.location.href='portfolio-affiches.php'" class="filter-btn">Affiches</button>
            </div>
        </div>
        <h2 class="portfolio-title">Nos Identités Visuelles</h2>
        
        <div class="portfolio-grid">
            <?php if (!empty($identites)): ?>
                <?php foreach ($identites as $identite): ?>
                    <?php 
                    $imagePath = $identite['image_path'] ?? '';
                    if (strpos($imagePath, 'admin/') !== 0 && strpos($imagePath, '/') !== 0) {
                        if (strpos($imagePath, 'images/') === 0) {
                            $imagePath = 'admin/' . $imagePath;
                        } else {
                            $imagePath = 'admin/images/Admin/identités/' . basename($imagePath);
                        }
                    }
                    $altText = htmlspecialchars($identite['client_name'] ?? 'Identité visuelle', ENT_QUOTES, 'UTF-8');
                    ?>
                    <div class="item identite">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p class="no-items">Aucune identité visuelle disponible pour le moment.</p>
            <?php endif; ?>
        </div>
    </div>
</section>
